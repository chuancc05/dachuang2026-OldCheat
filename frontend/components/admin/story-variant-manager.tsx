...
function emptyVariant(code: string, index: number): StoryVariant {
  return { id: `${code}-V${String(index).padStart(2, "0")}`, scenarioCode: code, title: "", persona: "", source: "模拟来电", premise: "", objective: "", pressureTactics: [], opening: "", fallbackLines: [], enabled: false, version: 1, updatedAt: new Date().toISOString() }
}
...
export function StoryVariantManager({ scenarios }: { scenarios: ScenarioOption[] }) {
  ...
  return (
    <main className="min-h-screen bg-slate-50 p-4 text-slate-900 md:p-8"><div className="mx-auto max-w-7xl space-y-4">
      <header className="rounded-2xl border bg-white p-5 shadow-sm"><h1 className="text-2xl font-bold">OldCheat 故事变体维护</h1><p className="mt-1 text-sm text-slate-600">维护受控剧本卡。管理令牌只保存在当前页面内存，刷新后清除。</p>
