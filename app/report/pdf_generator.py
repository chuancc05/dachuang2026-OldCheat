...
class PDFReportGenerator:
    ...
    def generate_report(self, 
        ...
        story.append(Paragraph("防范电信网络诈骗", self.title_style))
        story.append(Paragraph("多维心理评估与个性化报告", self.title_style))
        
        story.append(Spacer(1, 50))
        
        date_str = datetime.now().strftime("%Y-%m-%d %H:%M")
        meta_info = f"<b>受测者：</b>{user_name}<br/><b>测试场景：</b>{scene}<br/><b>生成时间：</b>{date_str}"
        ...
        # ================= 心理弱点雷达图 =================
        story.append(Paragraph("2. 心理维度雷达图", self.heading_style))
        story.append(Paragraph("基于在对话交互过程中的表现，系统对您的心理易感性进行了多维度打分，得分越高代表在该维度的风险越大：", self.body_style))
        ...
        if failure_points:
            for idx, point in enumerate(failure_points, 1):
                story.append(Paragraph(f"{idx}. {point}", self.body_style))
        else:
            story.append(Paragraph("本次测试中未检测到明显的关键失误，表现良好。", self.body_style))

        # ================= 个性化防骗建议 =================
        story.append(Paragraph("4. 个性化防骗建议", self.heading_style))
        story.append(Paragraph("基于上述评估，我们为您提取了风险最高的心理维度，并提供如下防范建议：", self.body_style))
        
        # 提取风险最高的2-3个维度
        top_risks = sorted(dimension_scores.items(), key=lambda x: x[1], reverse=True)[:3]
        
        for dim, score in top_risks:
            advice = self._get_advice_for_dimension(dim)
            story.append(Paragraph(f"<b>【{dim}】 (风险得分: {score:.1f}/10)</b>", self.body_style))
            story.append(Paragraph(advice, self.body_style))
